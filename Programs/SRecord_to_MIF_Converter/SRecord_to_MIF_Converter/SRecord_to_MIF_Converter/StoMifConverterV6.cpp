...
int main(int argc, char *argv[])
{
  ...
  std::string root = "C:\\M68kV6.0 - 640by480\\M68kV6.0 - 800by480 - (Verilog) For Students\\Programs\\";
  std::string hexFile = "M68kdebugmonitor.hex";
  std::string mifFile = "M68kdebugmonitor.mif";
  //std::string hexFile = "memtest.hex";
  //std::string mifFile = "memtest.mif";
  std::string hexFileFullPath = root + hexFile;
  std::string mifFileFullPath = root + mifFile; 

  const char *hexPath = hexFileFullPath.c_str();
  const char *mifPath = mifFileFullPath.c_str();

	if((SourceFilePtr = fopen(hexPath, "r")) == NULL) {
  //if((SourceFilePtr =  fopen("G:\\Courses\\Elec 465\\M68k Systems\\DE1\\Verilog\\M68kV6.0 - 800by480 - (Verilog) For Students\\Programs\\DebugMonitorCode\\M68kdebugmonitor.hex", "r")) == NULL)	{
	...
	if ((DestFilePtr = fopen(mifPath, "w")) == NULL) {
