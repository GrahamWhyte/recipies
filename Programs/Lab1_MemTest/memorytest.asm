...
; #define memTestStart    0x08020000
; #define memTestEnd      0x0802FFFF 
; #define memTestFailure  0x08023FFF
; /**********************************************************************************************
...
; unsigned int row, i=0, count=0, counter1=1;
       ...
       clr.l     -168(A6)
       move.l    #1,-164(A6)
; uint32_t *ramPtr, *failureLocation;
; uint32_t writeVar, readVar; 
; bool failure = FALSE; 
       clr.w     D5
; char c, text[150] ;
; int PassFailFlag = 1 ;
...
; printf("\r\nEnding memory address: %X ", ramPtr) ;
       ...
       addq.w    #8,A7
; // Inject failure
; failureLocation = (uint32_t*)memTestFailure; 
       move.l    #134365183,-160(A6)
; *failureLocation = 0xCCCCCCCC; 
       move.l    -160(A6),A0
       move.l    #-858993460,(A0)
; // Reset ramPtr and read from memory
; ramPtr = (uint32_t*)memTestStart;
       move.l    #134348800,D2
; while (ramPtr < memTestEnd)
main_4:
...
; }
       section   const
@memory~1_1:
       dc.b      13,10,83,116,97,114,116,105,110,103,32,109,101
       dc.b      109,111,114,121,32,97,100,100,114,101,115,115
       dc.b      58,32,37,88,32,0
@memory~1_2:
       dc.b      13,10,69,110,100,105,110,103,32,109,101,109
       dc.b      111,114,121,32,97,100,100,114,101,115,115,58
       dc.b      32,37,88,32,0
@memory~1_3:
...
@memory~1_6:
       dc.b      13,10,77,101,109,32,84,101,115,116,32,80,97
       dc.b      115,115,101,100,0
       section   bss
       xdef      _i
_i:
       ds.b      4
